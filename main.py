...
def json_to_filled_html_string(html_template: str, json_data: dict) -> str:
    ...
    for item in datax.get("data",{}).get("objDanMisDetails", []):
        row = {}
            ...
            if isinstance(value, list):
                for item in value:
                    ...
                    for k, v in result.items():
                        ...
                        # if value is dict with "value"
                        if isinstance(v, dict) and "value" in v:
                            flat[k.strip()] = v["value"]
    ...
    flat_data = flatten_llm_json(data)

    # merge (without breaking original)
                ...
                for key, value in obj["result"].items():
                    ...
                    if isinstance(value, dict):
                        if "value" in value:
                            ...
                            output[new_key] = value["remarks"]
                        elif "notes" in value:
                            output[new_key] = value["notes"]
                        else:
                            if isinstance(value,dict):
                                for i,j in value.items():
                                    output[f"{new_key}_{i}"] = j
                            else:
                                output[new_key] = ""
                    else:
...
def html_to_docx_bytes(input_html: str) -> bytes:
    ...
    highlight_rm_input_in_docx(doc, "<RM input>", "FFFF00")
...
def json_to_docx_bytes(html_template: str, json_data: dict) -> bytes:
    ...
    docx_bytes = html_to_docx_bytes(
        input_html=filled_html.replace('NOT_FOUND','RM to check and update').replace('Not_found','RM to check and update').replace('not_found','RM to check and update'),
    )
